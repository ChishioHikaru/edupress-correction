...
class Mailform {
	...
	public function __construct() {
		...
		$this->admin_mail_address    = $rm_admin_address;
	...
	// public referer_check
        //接続元をチェック。ドメイン外からのsubmitをブロックする
	...
	public function mail_send() {
		...
		if ( file_exists( dirname( __FILE__ ) .'/../addon/attachment/mail-multipart.php' ) ) {
			include( dirname( __FILE__ ) .'/../addon/attachment/mail-multipart.php' );
                ...
                //管理者へのメール
		$this->my_result = mb_send_mail( $this->admin_mail_address, $this->send_subject, $this->send_body, 'From:dam-system-info@edupress.net' );
		//error_log("エラー：473行目\n", 3, "./debug.log");
		//error_log($send_address_all, 3, "./debug.log");
		...
		if ( $this->reply_mail === 1 ) {
			...
			if ( $this->reply_mail_address === true ) {
				...
				$this->you_result = mb_send_mail( $this->mail_address, $this->thanks_subject, $this->thanks_body, $thanks_additional_headers );
                                error_log("エラー：484行目\n", 3, "./debug.log");
                                error_log($this->mail_address, 3, "./debug.log");
			}else{
